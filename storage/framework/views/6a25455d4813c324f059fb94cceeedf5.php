<div class="container-fluid">
    <div class="card mb-0">
        <div class="card-body">
            <h3 class="card-title"><?php echo e(__('Add Relation Type')); ?></h3>
            <form method="post" id="relation_type_form" class="form-horizontal" >
                <?php echo csrf_field(); ?>
                <div class="input-group">
                    <input type="text" name="relation_type_name" class="form-control" placeholder="<?php echo e(__('Relation Type')); ?>">
                    <input type="submit" id="relation_type_submit" class="btn btn-success" value=<?php echo e(trans("file.Save")); ?>>
                </div>
            </form>
        </div>
    </div>
</div>

<span class="relation_type_result"></span>
<div class="table-responsive">
    <table id="relation_type-table" class="table ">
        <thead>
        <tr>
            <th><?php echo e(__('Type name')); ?></th>
            <th class="not-exported"><?php echo e(trans('file.action')); ?></th>
        </tr>
        </thead>

    </table>
</div>


<div id="RelationTypeEditModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">

            <div class="modal-header">
                <h5 class="modal-title"><?php echo e(trans('file.Edit')); ?></h5>

                <button type="button" data-dismiss="modal" id="relation_type_close" aria-label="Close" class="close"><span
                            aria-hidden="true">×</span></button>
            </div>
            <span class="relation_type_result_edit"></span>

            <div class="modal-body">
                <form method="post" id="relation_type_form_edit" class="form-horizontal" enctype="multipart/form-data" >

                    <?php echo csrf_field(); ?>
                    <div class="col-md-6 form-group">
                        <label><?php echo e(__('Relation Type')); ?> *</label>
                        <input type="text" name="relation_type_name_edit" id="relation_type_name_edit"  class="form-control"
                               placeholder="<?php echo e(__('Type')); ?>">
                    </div>
                    <div class="col-md-4 form-group">
                        <input type="hidden" name="hidden_relation_type_id" id="hidden_relation_type_id" />
                        <input type="submit" name="relation_type_edit_submit" id="relation_type_edit_submit" class="btn btn-success" value=<?php echo e(trans("file.Edit")); ?> />
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
<?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/urtasker_crm/resources/views/settings/variables/partials/relation_type.blade.php ENDPATH**/ ?>