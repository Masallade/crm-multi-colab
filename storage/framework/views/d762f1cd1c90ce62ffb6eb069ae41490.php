        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                    ...
                    <div class="section-content">
                        <div class="form-group">
                            ...
                            <?php if(!empty($sectionData)): ?>
                                ...
                                <option value="<?php echo e($index); ?>"><?php echo e($section['section']['name'] . "  /  " . $section['corresponding_designation_name']."  /  " . $section['corresponding_department_name']); ?></option>                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php endif; ?>
                            </select>
                        </div>
                        <div id="sectionWeightageDisplay" class="alert alert-info mt-2" style="font-weight:bold;">
                            <!-- Weightage will be shown here -->
                        </div>
                    </div>
                </div>
                ...
                <form action="<?php echo e(route('submitEmployeeAppraisal')); ?>" method="post" id="submitForm">
                    ...
                    <input type="hidden" name="departement_id" id="departmentIdInput">
                    <input type="hidden" name="designation_id" id="designationIdInput">
                    ...
                    <div class="form-group">
                        <label for="evaluator_comment">Evaluator Comment</label>
                        <textarea class="form-control" name="evaluator_comment" id="evaluator_comment" rows="3" placeholder="Enter your comments about the employee (optional)"></textarea>
                    </div>
...
const designationData = <?php echo json_encode($designations, 15, 512) ?>;
const designationNameToIds = <?php echo json_encode($designationNameToIds, 15, 512) ?>;
...
function updateFormContent(sectionIndex) {
    ...
    $('#departmentIdInput').val(section.corresponding_department_id || '');
    $('#designationIdInput').val(
        Array.isArray(section.corresponding_designation_ids) 
            ? section.corresponding_designation_ids[0] 
            : section.corresponding_designation_ids || ''
    );
    ...
    console.log("✅ real departmentId:", departmentId); // Log the department ID

    // 🔍 Get the list of valid designation IDs from the section
    let designationIds = section.corresponding_designation_ids;
    console.log("real ✅ designationIds (original):", designationIds); 
    ...
    // Show section weightage
    if (section.section && section.section.weightage !== undefined) {
        $('#sectionWeightageDisplay').html('<i class="fas fa-balance-scale"></i> Section Weightage: <span style="color:#007bff">' + section.section.weightage + '%</span>');
    } else {
        $('#sectionWeightageDisplay').html('');
    }
}
...
</script><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/crm/resources/views/performance/appraisal/create-modal.blade.php ENDPATH**/ ?>