...
class AttendanceController extends Controller
{
    ...
    public $date_range = [];
    public $work_days = 0;
    protected $shift_in_column;
    protected $shift_out_column;
    protected $selected_date_for_clockout;
    protected $time_late_for_nightShift=null;
    protected $shift_out_for_overtime_early;
    protected $overtime_check;
    
    
    
        
    /**
     * Get the column name for shift in based on a date
     * 
     * @param string|Carbon $date The date to get shift_in column for
     * @return string Column name for the day's shift_in
     */
    function fetchSelectedDayShiftIn($date = null) {
        // Use provided date or fallback to current date
        $date = $date ? Carbon::parse($date) : Carbon::now();
        
        // Get the day name from the date
        $day_name = $date->format('l'); // Full textual representation of the day
    ...
    /**
     * Get the column name for shift out based on a date
     * 
     * @param string|Carbon $date The date to get shift_out column for
     * @return string Column name for the day's shift_out
     */
    function fetchSelectedDayShiftOut($date = null) {
        // Use provided date or fallback to current date
        $date = $date ? Carbon::parse($date) : Carbon::now();
        ...
        $weekdays = (object) [
            'Monday' => 'monday_out',
            'Tuesday' => 'tuesday_out',
            'Wednesday' => 'wednesday_out',
            'Thursday' => 'thursday_out',
            'Friday' => 'friday_out',
            'Saturday' => 'saturday_out',
            'Sunday' => 'sunday_out',
        ];
    ...
    function returnClockOut($shift_out, $records) {
    Log::info($records);
    
    $time_parts = explode(':', $shift_out);
    // Convert the first two elements (hours and minutes) to integers
    $sign_out_hour = (int)$time_parts[0] + 4;   // Hours as integer
    $sign_out_minute = (int)$time_parts[1]; 
    
    $valid_records = [];
    
    foreach ($records as $record) {
        // Parse clock_in time (converting from 12-hour to 24-hour format)
        $clock_in_time = $record->clock_in;     
        $time_parts = explode(' ', $clock_in_time);
        
        // Skip PM records
     
        
        if (strtoupper($time_parts[1]) !== 'AM') {
        ...
        $time = explode(':', $time_parts[0]);
        $period = $time_parts[1];
        
        $clock_in_hour = (int)$time[0];
        $clock_in_minute = (int)$time[1];
        
        if ($sign_out_hour >= $clock_in_hour || $clock_in_hour==12 ) {
            if ($sign_out_hour==$clock_in_hour && $sign_out_minute >= $clock_in_minute) {
        ...
        Log::info('Valid records:', $valid_records);

    
    if (!empty($valid_records)) {
    
    Log::info("Valid records is not empty");
        
        $this->time_late_for_nightShift= end($valid_records)->clock_out;
        
        return end($valid_records)->clock_out;
    }
    Log::info("Valid records is empty");
    return null; // or handle the case when no valid records are found
}
...
protected function returnOvertimeOrEarlyLeaving($shift_out, $clock_out, $selectedDate)
{
    
   
    $shiftInTime = $shift_out; // e.g., "01:30 AM"
    $clockOutTime = $clock_out; // e.g., "08:08:00 PM"
    
    // Parse meridiem indicators
    $shiftInMeridiem = strtoupper(Carbon::parse($shiftInTime)->format('A'));
    $clockOutMeridiem = strtoupper(Carbon::parse($clockOutTime)->format('A'));
    
    // Log input parameters
    Log::info("Overtime Calculation Parameters", [
        'shift_out' => $shift_out,
        'clock_out' => $clock_out,
        'selected_date' => $selectedDate,
        'shift_meridiem' => $shiftInMeridiem,
        'clockout_meridiem' => $clockOutMeridiem
    ]);
    
    // Determine shift date (handle night shifts)
    $shiftInDate = $selectedDate;
    
    // Case 1: Shift starts PM and ends AM (next day)
    if ($shiftInMeridiem === 'PM' && $clockOutMeridiem === 'AM') {
        $shiftInDate = Carbon::parse($selectedDate)->subDay()->format('Y-m-d');
    }
    // Case 2: Shift starts AM and ends PM (same day) - no adjustment needed
    // Case 3: Shift starts AM and ends AM (next day) - night shift
    elseif ($shiftInMeridiem === 'AM' && $clockOutMeridiem === 'PM') {
        $shiftInDate = Carbon::parse($selectedDate)->addDay()->format('Y-m-d');
    }
    
    // Create datetime objects
    $shiftInDateTime = Carbon::parse("$shiftInDate $shiftInTime");
    $clockOutDateTime = Carbon::parse("$selectedDate $clockOutTime");
    
    // Log datetime objects
    Log::info("Datetime Objects", [
        'shift_in_datetime' => $shiftInDateTime->format('Y-m-d H:i:s'),
        'clock_out_datetime' => $clockOutDateTime->format('Y-m-d H:i:s'),
        'timezone' => $shiftInDateTime->timezoneName,
        'shift_date_adjustment' => $shiftInDate !== $selectedDate ? 'adjusted' : 'same day'
    ]);
    
    // Calculate raw difference in seconds
    $diffInSeconds = $clockOutDateTime->diffInRealSeconds($shiftInDateTime, false);
    Log::info("Raw Difference", ['seconds' => $diffInSeconds]);
    
    // Calculate absolute difference components
    $absoluteSeconds = abs($diffInSeconds);
    ...
    Log::info("Overtime Calculation Result", [
        ...
        'interpretation' => $diffInSeconds > 0 ? 'Overtime' : 'Early Leaving',
        'hours' => $hours,
        'minutes' => $minutes,
        'seconds' => $seconds
    ]);
    ...
    public function index(Request $request)
    {
        ...
        $office_shift = $request->office_shiftid;
        // dd($office_shift);
        ...
        $this->selected_date_for_clockout=$selected_date;

        $day = strtolower(Carbon::parse($request->filter_month_year)->format('l')) . '_in' ?? strtolower(now()->format('l')) . '_in';

       $this->shift_in_column = $this->fetchSelectedDayShiftIn($selected_date);
       $this->shift_out_column = $this->fetchSelectedDayShiftOut($selected_date);
        
        Log::info('Shift columns fetched', [
            'selected_date' => $selected_date,
            'shift_in_column' => $this->shift_in_column,
            'shift_out_column' => $this->shift_out_column,
        ]);
        ...
        if (request()->ajax()) {
            ...
            if (!($logged_user->can('daily-attendances'))) {
                ...
                $employeeQuery = Employee::with([
                    ...
                    ->select('id', 'company_id', 'first_name', 'last_name', 'office_shift_id','staff_id')
                    ->where('joining_date', '<=', $selected_date)
                    ->where('is_active', 1)
                    ->whereNull('exit_date');
                    if (!empty($office_shift)) {
                        $employeeQuery->whereIn('office_shift_id', $office_shift);
                    }
            ...
            return datatables()->of($employee)
                ...
                ->addColumn('staff_code', function ($employee) {
                    return $employee->staff_id;
                })
                ->addColumn('employee_name', function ($employee) {
                    return $employee->full_name;
                })
                ->addColumn('shift_name', function ($employee) {
                    return $employee->officeShift->shift_name ?? 'N/A'; 
                })
          ...
          ->addColumn('clock_in', function ($employee) use ($day) {
                try {
                    if ($employee->employeeAttendance->isEmpty()) {
                        return '---';
                    } else {
                        // Get the shift start time for this employee on this day
                        $shift_start = $employee->officeShift->$day ?? null;
                        
                        if ($shift_start && !empty($shift_start)) {
                            // Try to find valid clock-in records
                            $valid_clock_in = null;
                            ...
                            foreach ($employee->employeeAttendance as $attendance) {
                                ...
                                if (empty($attendance->clock_in)) continue;
                                
                                // Keep track of the earliest clock-in regardless of criteria
                                if ($earliest_clock_in === null || $attendance->clock_in < $earliest_clock_in) {
                                    $earliest_clock_in = $attendance->clock_in;
                                }
                                
                                // Try to parse the times safely
                                try {
                                    // Parse shift start time and attendance clock-in time
                                    $shift_start_time = Carbon::parse($shift_start);
                                    $early_limit = (clone $shift_start_time)->subHours(4);
                                    $clock_in_time = Carbon::parse($attendance->clock_in);
                                    
                                    // Check if this clock-in is within our valid window
                                    if ($clock_in_time->greaterThanOrEqualTo($early_limit)) {
                                        if ($valid_clock_in === null || $clock_in_time->lessThan(Carbon::parse($valid_clock_in))) {
                                            $valid_clock_in = $attendance->clock_in;
                                        }
                                    }
                                } catch (\Exception $e) {
                            ...
                            // Return the valid clock-in if found, otherwise fallback to earliest
                            return $valid_clock_in ?? $earliest_clock_in ?? '---';
                        } else {
                            // If no shift information, just return the earliest clock-in
                            $attendance_row = $employee->employeeAttendance->sortBy('clock_in')->first();
                            return $attendance_row ? $attendance_row->clock_in : '---';
                        }
                    }
                } catch (\Exception $e) {
                    // Log the error for debugging
                    \Log::error('Error in clock_in column: ' . $e->getMessage());
               ...
               ->addColumn('clock_out', function ($employee) {
                        try {
                            // Debug logging
                            Log::info('Clock Out Processing', [
                                'employee_id' => $employee->id,
                                'office_shift_id' => $employee->office_shift_id,
                                'selected_date' => $this->selected_date_for_clockout ?? 'not_set',
                                'shift_in_column' => $this->shift_in_column ?? 'not_set',
                                'shift_out_column' => $this->shift_out_column ?? 'not_set'
                            ]);
                    
                            // Safely get shift information if available
                            $shift_in = null;
                            $shift_out = null;
                            
                            if (!empty($employee->office_shift_id)) {
                                $shift = office_shift::find($employee->office_shift_id);
                                
                                if ($shift) {
                                    // Safely access dynamic properties
                                    $shift_in = isset($this->shift_in_column) ? $shift->{$this->shift_in_column} : null;
                                    $shift_out = isset($this->shift_out_column) ? $shift->{$this->shift_out_column} : null;
                                    
                                    $this->shift_out_for_overtime_early =$shift_out;
                                    
                                    Log::debug('Shift Times', [
                                        'shift_in' => $shift_in,
                                        'shift_out' => $shift_out
                                    ]);
                                }
    ...
    $isOvernightShift = (strpos($shift_in, 'PM') !== false && strpos($shift_out, 'AM') !== false);
                            }
                            
                            
                            if ($isOvernightShift) {
                                
                                Log::debug('YES NGIHT SHIFT');
                                if (Carbon::parse($this->selected_date_for_clockout)->isToday()) {
                                    ...
                                    $selected_date_plus_one = Carbon::parse($this->selected_date_for_clockout)->addDay()->format('Y-m-d');
                                    
                                    Log::debug('Previous Date');
                                    Log::debug($selected_date_plus_one);
                                    
                            try {
                                $attendanceRecords = DB::table('attendances')
                                                     ->select('clock_in', 'clock_out','overtime')  // Only select these columns
                                                     ->where('attendance_date', $selected_date_plus_one)
                                                     ->where('employee_id', $employee->id)
                                                     ->get();
                            
                                if ($attendanceRecords->isNotEmpty()) {
                                    Log::debug('Attendance records found:', ['records' => $attendanceRecords->toArray()]);
                                } else {
                                    
                                    // $this->time_late_for_nightShift=null;
                                    // $this->shift_out_for_overtime_early=null;
                                    Log::debug('No attendance records found for employee', [
                                            
                                        'employee_id' => $employee->id,
                                        'date' => $selected_date_plus_one
                                    ]);
                                }
                                
                               $result_clock_out= $this->returnClockOut($shift_out,$attendanceRecords);
                                
                                if($result_clock_out){
                                return $result_clock_out;
                                }
                                else{
                            ...
                            $attendance_row = $employee->employeeAttendance->last();
                            Log::info('this->time_late_for_nightShift');
                            Log::info($this->time_late_for_nightShift);
                            $this->time_late_for_nightShift=$attendance_row->clock_out;
                            
                            Log::info('$attendance_row->clock_out ?? Working');
                            Log::info($attendance_row->clock_out);
                            return $attendance_row->clock_out ?? 'Working';
                                ...
                                return collect(); // Return empty collection instead of null
                                 ...
                                 Log::debug('No NGIHT SHIFT');

                             }
                            
                    
                            // Handle attendance data with fallbacks
                            if ($employee->employeeAttendance->isEmpty()) {
                                return '---';
                            }
                    
                            $attendance_row = $employee->employeeAttendance->last();
                            // $this->time_late_for_nightShift=$attendance_row->overtime;
                            ...
                            Log::error('Clock Out Column Error', [
                ...
                // })
                   ...
                   ->addColumn('early_leaving', function ($employee) {
                      ...
                      if($this->time_late_for_nightShift!=null){
                             Log::info("time_late_for_nightShift not null");
                            
                            // return $this->time_late_for_nightShift;
                            
                            $this->overtime_check=  $this->returnOvertimeOrEarlyLeaving($this->shift_out_for_overtime_early, $this->time_late_for_nightShift, $this->selected_date_for_clockout);
                            
                            
                            if (strpos($this->overtime_check, '+') === 0) {
                                return substr($this->overtime_check, 1);
                            }
                            else{
                                return "00:00:00";
                            }
                ...
                // })
                 ...
                 ->addColumn('overtime', function ($employee) {
                    if ($employee->employeeAttendance->isEmpty()) {
                        ...
                        Log::info("time_late_for_nightShift is null");    
                        $lastOvertime = DB::table('attendances')
                                                     ->select('clock_out','overtime')  // Only select these columns
                                                     ->where('attendance_date', $this->selected_date_for_clockout)
                                                     ->where('employee_id', $employee->id)
                                                     ->first();
                        ...
                        if($lastOvertime){
                             if($lastOvertime->clock_out){
                            
                        Log::info("lastOvertime");
                        Log::info($lastOvertime->overtime);
                            
                         $this->overtime_check=  $this->returnOvertimeOrEarlyLeaving($this->shift_out_for_overtime_early,$lastOvertime->clock_out, $this->selected_date_for_clockout);
                ...
                ->addColumn('percentage', function ($employee) use ($request) {
                    try {
                        $selectedDate = $request->filter_month_year 
                            ...
                            : Carbon::now(); // Use current date if null
                    ...
                    $percentage = $shiftDuration > 0 
                        ? round(($totalWorkHours / $shiftDuration) * 100, 2) . '%' 
                        : '0%';
        ...
        $office_shift = office_shift::get();
        return view('timesheet.attendance.attendance', ['office_shift' => $office_shift]);
        // }
...
// dd($employee_ids);
            ...
            if (!$request->company_id && !$request->department_id && empty($employee_ids)) {
                ...
                $employeeQuery = Employee::with([
                    ...
                    ->select('id', 'company_id', 'first_name', 'last_name', 'office_shift_id', 'joining_date','staff_id')
                    ->where('is_active', '=', 1);
...
// ✅ Check if officeShift exists before accessing the property
if ($present->isNotEmpty()) {
    $this->work_days++;
    return 'P';
} elseif (!$emp->officeShift || !isset($emp->officeShift->$day)) {
    return 'O'; // Return 'O' if officeShift is missing or day is not set
    ...
    public function calculateMonthlyPercentage($employee)
    {
        ...
        $workDays = $this->work_days;  // This is set during the DataTable process
        ...
        // Calculate total shift hours minus 1-hour break daily (for work days)
        $totalShiftHours = 0;
        foreach (range(1, 31) as $day) {
            $date = Carbon::parse($day . '-' . now()->format('m-Y'));
        ...
        // Calculate percentage based on work days
        $totalPossibleShiftHours = $workDays * 7.5;  // Assuming 7.5 hours per day worked
    
        $percentage = $totalPossibleShiftHours > 0
            ? round(($totalWorkHours / $totalPossibleShiftHours) * 100, 2) . '%'
            : '0%';
        ...
        if ($logged_user->can('edit-attendance')) {
            if (request()->ajax()) {
                ...
                return datatables()->of($employee_attendance)
                    ...
                    ->addColumn('staff_id', function ($row) {
                        return $row->employee ? $row->employee->staff_id  : 'N/A';
                    })
                    ->addColumn('employee_name', function ($row) {
                    ...
                    ->addColumn('shift_name', function ($row) {
                        return optional($row->employee->officeShift)->shift_name ?? 'N/A';
                    })
...
public function getEmployeeStats(Request $request)
{
    ...
    {
        $employee_ids = explode(',', $request->employee_ids);
        $attendance_date1 = Carbon::parse($request->attendance_date1)->format('Y-m-d');
        $attendance_date2 = Carbon::parse($request->attendance_date2)->format('Y-m-d');

        $data = Attendance::with('employee')
            ->whereIn('employee_id', $employee_ids)
            ->whereDate('attendance_date', '>=', $attendance_date1)
            ->whereDate('attendance_date', '<=', $attendance_date2)
            ->get();

        $pdf = PDF::loadView('timesheet.updateAttendance.pdf', compact('data'));
        
        return $pdf->download("attendance_report_{$attendance_date1}_to_{$attendance_date2}.pdf");
    }
    ...
    public function downloadCSV(Request $request)
    {
        ...
        $callback = function() use ($data) {
            ...
            fputcsv($file, ['Employee', 'In Time', 'Out Time', 'Date']);
        ...
        $headers['Content-Disposition'] = 'attachment; filename="attendance_report_' . $attendance_date1 . '_to_' . $attendance_date2 . '.csv"';
