...
class FileManagerController extends Controller
{
			...
			if (request()->ajax())
			{
				return datatables()->of(FileManager::with('department:id,department_name', 'AddedBy:id,username')->get())
					...
					->addColumn('action', function ($data) {
						...
						// Hide download button for role_users_id == 2 or 4
						...
						$button .= '<a class="btn btn-success btn-sm" href="' . route('files.view_file', $data->id) . '"><i class="dripicons-preview"></i></a>';
						$button .= '&nbsp;&nbsp;';
					
						// Add edit button for users with 'edit-file_manager' permission, but not for role_users_id == 2
						if (auth()->user()->can('edit-file_manager') && auth()->user()->role_users_id != 2) {
							$button .= '<button type="button" name="edit" id="' . $data->id . '" class="edit btn btn-primary btn-sm"><i class="dripicons-pencil"></i></button>';
							$button .= '&nbsp;&nbsp;';
						}
					
						// Add delete button for users with 'delete-file_manager' permission
	...
	public function view_file($id){
		...
		$download_path = url("uploads/file_manager/" . $file_path . '.' . $file_extension);
		if($download_path){
			$file_data = $download_path;
		}else{
			$file_data = 0;
		}
		
		return view('file_manager.file_view', compact('file_data'));
	}
