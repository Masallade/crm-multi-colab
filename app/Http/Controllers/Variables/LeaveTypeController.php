...
class LeaveTypeController {
	...
	public function index()
	{
		if (request()->ajax())
		{
            $data = LeaveType::select('id', 'leave_type')->get();
    ...
    private function leaveTypeManageUpdate_($request, $leaveTypeId)
    {
    ...
    private function leaveTypeManageUpdate($request, $leaveTypeId)
    {
        ...
        $leaveTypes = LeaveType::select('id','leave_type')->get(); // Remove 'allocated_day' from here
    
        foreach ($employees as $employee) {
            if ($employee->employeeLeaveTypeDetail) {
                $leave_type_details = unserialize($employee->employeeLeaveTypeDetail->leave_type_detail);
                
                // Preserve existing leave details
                $dataLeaveType = [];
    
                foreach ($leave_type_details as $key => $value) {
                    if ($value["leave_type_id"] == $leaveTypeId) {
                        // Only update leave_type, preserve other values
                        $value['leave_type'] = $leaveTypes->where('id', $leaveTypeId)->first()->leave_type ?? $value['leave_type'];
                    }
                    // Store the preserved data
                    $dataLeaveType[$key] = $value;
                }
    
                // Update the database without modifying allocated_day or remaining_allocated_day
                EmployeeLeaveTypeDetail::updateOrCreate(
                    ['employee_id' => $employee->id],
                    ['leave_type_detail' => serialize($dataLeaveType)]
                );
