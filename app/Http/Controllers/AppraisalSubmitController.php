...
class AppraisalSubmitController extends Controller
{
    ...
    function submitEmployeeAppraisal(Request $request) {
        ...
        // Add evaluator comment to data if present
        if ($request->has('evaluator_comment')) {
            $data['evaluator_comment'] = $request->input('evaluator_comment');
        }
    
        // Save appraisal using model function
        $appraisal = Appraisal::saveAppraisal($data, (int)$data["designation_id"] ,(int)$data["departement_id"]);
