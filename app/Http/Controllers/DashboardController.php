		...
		if ($designations)
		{
			foreach ($designations as $key => $desig)
			{
				...
				if ($key === null) {
					...
					$firstDesig = $desig->first();
					if ($firstDesig && $firstDesig->designation && $firstDesig->department) {
						// Append department name to designation name
						$desig_name_array[] = $firstDesig->designation->designation_name . ' (' . $firstDesig->department->department_name . ')';
					} elseif ($firstDesig && $firstDesig->designation) {
						$desig_name_array[] = $firstDesig->designation->designation_name;
					} else {
						$desig_name_array[] = __('No Designation');
					}
				}
...
// dd($desig_name_array);
		...
		// Role-aware pending leave count
		$user = auth()->user();
		if ($user->role_users_id == 1) {
			$leaveCountPending = Leave::where('status', 'pending')->count();
		} elseif ($user->role_users_id == 6) {
			// HR sees same pending count as list view → TL not acted yet
			$leaveCountPending = Leave::where('status', 'pending')->where('is_tl_action', 0)->count();
		} elseif ($user->role_users_id == 4) {
			$leaveCountPending = Leave::whereHas('employee', function ($query) use ($user) {
				$query->where('team_lead', $user->id)
					->orWhere('id', $user->id);
			})->where('status', 'pending')->where('is_tl_action', 0)->count();
		} elseif ($user->role_users_id == 2) {
			$leaveCountPending = Leave::whereHas('employee', function ($query) use ($user) {
				$query->where('id', $user->id);
			})->where('status', 'pending')->where('is_tl_action', 0)->count();
		} elseif ($user->role_users_id == 9) {
			$loggedEmployee = Employee::find($user->id);
			if ($loggedEmployee) {
				$leaveCountPending = Leave::where(function ($q) use ($loggedEmployee, $user) {
					$q->where('department_id', $loggedEmployee->department_id)
						->orWhereHas('employee', function ($sub) use ($user) {
							$sub->where('team_lead', $user->id)
								->orWhere('id', $user->id);
						});
				})->where('status', 'pending')->where('is_tl_action', 0)->count();
			} else {
		...
		return view('dashboard.admin_dashboard', compact('employees', 'attendance_count', 'leave_count', 'total_expense_raw', 'total_deposit_raw', 'total_expense', 'total_deposit', 'total_salaries_paid',
            ...
            'versionUpgradeData','leaveCountPending'
        ));
	}
	...
	public function employeeDashboard(Request $request)
	{
	   ...
	   // notifyUser(240, 'Leave Request', 'A new leave has been applied.');
		$user = auth()->user();
		check_feedback_email($user->id);
// 		dd(check_feedback_email($user->id));
		if($user->role_users_id != 6){
		// Role-aware pending leave count for employee dashboard
		if ($user->role_users_id == 1) {
			$leaveCountPending = Leave::where('status', 'pending')->count();
		} elseif ($user->role_users_id == 6) {
			$leaveCountPending = Leave::where('status', 'pending')->where('is_tl_action', 0)->count();
	...
	// dd($desig_name_array);
// 			$attendance_count = Attendance::where('attendance_date', now()->format('Y-m-d'))->distinct('employee_id')->count();
			$attendance_count = Attendance::where('attendance_date', now()->format('Y-m-d'))
			...
			// Role-aware pending leave count (fallback block)
			$user = auth()->user();
			if ($user->role_users_id == 1) {
				$leaveCountPending = Leave::where('status', 'pending')->count();
			} elseif ($user->role_users_id == 6) {
