...
class EmployeeController extends Controller
{
    ...
    public function employees_chart()
    {
        ...
        $validExitDateFilter = function ($query) use ($currentDate) {
            $query->whereNull('exit_date')
                  ->orWhereDate('exit_date', '>=', $currentDate);
        };
    
        if (in_array($loggedUser->role_users_id, [1, 6, 5])) {
            // Admin-like roles: get top-level employees (those without a team lead)
            $employees = Employee::with(['children', 'designation', 'department', 'user'])
                ->whereNull('team_lead')
                ->where($validExitDateFilter)
                ->get();


    
        } elseif ($loggedUser->role_users_id == 4) {
            // Team Lead: get themselves + team
    
            $loggedUserData = Employee::with(['children', 'designation', 'department', 'user'])
                ->where('id', $loggedUser->id)
                ->where($validExitDateFilter)
                ->first();
    
            $teamMembers = Employee::with(['children', 'designation', 'department', 'user'])
                ->where('team_lead', $loggedUser->id)
                ->where($validExitDateFilter)
                ->get();
    
            $employees = collect([$loggedUserData])->filter()->merge($teamMembers);
        } else {
            // Regular user: show self only
            $employees = Employee::with(['children', 'designation', 'department', 'user'])
                ->where('id', $loggedUser->id)
                ->where($validExitDateFilter)
                ->get();
    ...
    public function index(Request $request)
    {
        ...
        if ($logged_user->can('view-details-employee')) {
            ...
            if (request()->ajax()) {
                ...
                return datatables()->of($employees)
                    ...
                    ->addColumn('last_appraisal_date', function ($row) {
                        return $row->last_appraisal_date;
                    })
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        if (auth()->user()->can('view-details-employee')) {
                            $button .= '<a href="employees/'.$row->id.'" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>';
                        }
        ...
        $employees_data = Employee::with(['children', 'designations', 'departments','user'])->where('email','faisalc@basepracticesupport.co.uk')->get();
        
        $hierarchy = $this->buildHierarchy($employees_data);
        // dd($hierarchy);
        // Convert the entire hierarchy into a JSON string for easy use in JavaScript
        $jsonHierarchy = json_encode($hierarchy);
        $employees = $this->getEmployees($request, $currentDate);
            return view('employee.index', compact('companies', 'roles','team_lead','jsonHierarchy','employees'));
        } else {
    ...
    public function changeTeamLead(Request $request, $id)
    {
        $request->validate([
            'new_team_lead_id' => 'required|exists:employees,id',
        ]);
    
        // Step 1: Find all employees who report to the current team lead (i.e., $id)
        $subordinates = Employee::where('team_lead', $id)->get();
    
        // Step 2: Update their team lead to the new one
        foreach ($subordinates as $employee) {
            $employee->team_lead = $request->new_team_lead_id;
            $employee->save();
        }
    
        return response()->json(['success' => true, 'message' => 'Team lead changed for all related employees.']);
    }
    
    


    protected function getxEmployees($request, $currentDate)
    {
        $loggedUser = auth()->user(); // Get the logged-in user
        $employees = null;
    ...
    public function x_employee(Request $request)
    {
        $logged_user = auth()->user();
        if ($logged_user->can('view-details-employee')) {
            $companies = Company::select('id', 'company_name')->get();
            ...
            if (request()->ajax()) {
                ...
                $employees = $this->getxEmployees($request, $currentDate);
        ...
        $jsonHierarchy = json_encode($hierarchy);
            ...
            return view('employee.xemployee', compact('companies', 'roles','team_lead','jsonHierarchy'));
        } else {
            return response()->json(['success' => __('You are not authorized')]);
        }
    }
