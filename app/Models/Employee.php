...
class Employee extends Model
{
	...
	protected $fillable = [
		'id','team_lead', 'first_name','last_name','staff_id','email','last_appraisal_date','appraisal_date_mail','contact_no','date_of_birth','gender','status_id','office_shift_id','salary_id','location_id','designation_id', 'company_id', 'department_id','is_active',
		'role_users_id','permission_role_id','joining_date','probation_period','exit_date','marital_status','address','city','state','country','zip_code','cv','skype_id','fb_id',
		...
		'travel_id','promotion_id','complain_id','warning_id','termination_id','attendance_type','total_leave','remaining_leave','pension_type','pension_amount','appr'];


		public function children() {
			return $this->hasMany(Employee::class, 'team_lead', 'id')
				->where(function ($query) {
					$query->whereNull('exit_date')
						  ->orWhere('exit_date', '>=', now()->toDateString())
	...
	public static function getEmployeeDesDepartId($employeeId){
		return self::where('id',$employeeId)->first(['designation_id', 'department_id']);
	}
