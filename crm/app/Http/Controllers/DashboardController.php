...
// dd($desig_name_array);
		...
		$leaveCountPending = Leave::where('status', 'pending') // Filter for pending leaves
			->count();
...
// 		dd(check_feedback_email($user->id));
		...
		$leaveCountPending = Leave::whereHas('employee', function ($query) use ($user) {
			$query->where('team_lead', $user->id)
				->orWhere('id', $user->id); // Include the logged-in user's own leave
			})->where('status', 'pending') // Filter for pending leaves
			->count();
