        ...
        <div class="container-fluid mb-3">
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('store-promotion')): ?>
                <button type="button" class="btn btn-info" name="create_record" id="create_record"><i
                            class="fa fa-plus"></i> <?php echo e(__('Update Promotion')); ?></button>
            <?php endif; ?>
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('store-promotion')): ?>
                ...
                <div class="modal-body">
                    ...
                    <form method="post" id="sample_form" class="form-horizontal">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label><?php echo e('Current Designation'); ?></label>
                                    <div id="current_designation" class="form-control" style="background-color:#f8f9fa; padding: 8px; border-radius: 4px;">
                                        <span class="text-muted">Select employee to see current designation</span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label><?php echo e('Select New Designation'); ?></label>
...
<script type="text/javascript">
    (function($) {
        ...
        $(document).on('change', '.employee', function() {
            ...
            if(employee_id) {
                $.ajax({
                    url: "<?php echo e(route('get_employee_designation')); ?>",
                    type: "GET",
                    data: {employee_id: employee_id},
                    success: function(data) {
                        if(data.designation) {
                            $('#current_designation').html('<strong>' + data.designation + '</strong>');
                        } else {
                            $('#current_designation').html('<span class="text-danger">No designation found</span>');
                        }
                    },
                    error: function() {
                        $('#current_designation').html('<span class="text-danger">Error fetching designation</span>');
                    }
                });
            } else {
                $('#current_designation').html('<span class="text-muted">Select employee to see current designation</span>');
            }
        });
        ...
        $(document).on('click', '.edit', function () {
            ...
            $.ajax({
                ...
                success: function (html) {
                    ...
                    // Show current designation when editing
                    if(html.data.employee_id) {
                        $.ajax({
                            url: "<?php echo e(route('get_employee_designation')); ?>",
                            type: "GET",
                            data: {employee_id: html.data.employee_id},
                            success: function(data) {
                                if(data.designation) {
                                    $('#current_designation').html('<strong>' + data.designation + '</strong>');
                                } else {
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layout.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Xampp\htdocs\crm_latest_backup_13_04_2025\crm\resources\views/core_hr/promotion/index.blade.php ENDPATH**/ ?>