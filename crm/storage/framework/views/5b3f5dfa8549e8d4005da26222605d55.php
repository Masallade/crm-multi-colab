...
function formatFullRecord(data) {
    ...
    var html = '<div class="sections-container">';
    data.forEach(function(section, index) {
        html += '<div class="section mb-3 p-3 border rounded">';
        html += '<h6 class="section-title font-weight-bold">Section ' + (index + 1) + ': ' + (section.section_name || '') + '</h6>';
        
        // Add section details
        for (var key in section) {
            if (section.hasOwnProperty(key)) {
                if (key === 'performance_indicators') {
                    // Special handling for performance indicators
                    html += '<div class="performance-indicators mt-3">';
                    html += '<h6 class="font-weight-bold">Performance Indicators:</h6>';
                    html += '<ul class="list-group">';
                    
                    for (var indicator in section[key]) {
                        html += '<li class="list-group-item d-flex justify-content-between align-items-center">';
                        html += indicator;
                        html += '<span class="badge badge-primary badge-pill">' + section[key][indicator] + '</span>';
                        html += '</li>';
                    }
                    ...
                    html += '<div class="section-detail mb-2">';
                    html += '<strong>' + key + ':</strong> ';
                    if (typeof section[key] === 'object' && section[key] !== null) {
                        html += JSON.stringify(section[key], null, 2);
                    } else {
                        html += section[key];
                    }
                    html += '</div>';
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layout.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/baselinepractice/public_html/v2/crm/resources/views/performance/appraisal/index.blade.php ENDPATH**/ ?>