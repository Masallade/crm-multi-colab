...
use App\Http\Controllers\Variables\EmployeeaddLeaveController;
...
use App\Http\Controllers\OffDeskHoursController;
use App\Http\Controllers\ExeWorkingReportController;
use App\Http\Controllers\AhhamController;




Route::post('/send-push', [PushNotificationController::class, 'sendPush'])->name('sendPush');
Route::post('/save-token', [PushNotificationController::class, 'saveToken'])->name('saveToken');
...
Route::group(['middleware' => ['XSS','checkDataTable']], function () use ($isCrmModuleExist) {
    ...
    Route::prefix('staff')->group(function () {
        ...
        Route::get('employee-tracking', [AttendanceController::class, 'employee_tracking'])->name('employee_tracking');
        Route::get('/get-employee-stats', [AttendanceController::class, 'getEmployeeStats']);
        
        Route::get('ex-employees', [EmployeeController::class, 'x_employee'])->name('x_employee');
        Route::prefix('employees')->group(function () {
            ...
            // Define a specific route for employees_chart
        Route::get('chart', [EmployeeController::class, 'employees_chart'])->name('employees.chart');
    ...
    Route::prefix('core_hr')->group(function () {
        ...
        // Resignation approval routes
        Route::post('resignations/{id}/hr-approve', [ResignationController::class, 'hrApprove'])->name('resignations.hr-approve');
        Route::post('resignations/{id}/admin-approve', [ResignationController::class, 'adminApprove'])->name('resignations.admin-approve');
        Route::post('resignations/{id}/hr-reject', [ResignationController::class, 'hrReject'])->name('resignations.hr-reject');
        Route::post('resignations/{id}/admin-reject', [ResignationController::class, 'adminReject'])->name('resignations.admin-reject');
    ...
    Route::prefix('organization')->group(function () {
        ...
        Route::post('locations/update/{location}', [LocationController::class, 'update'])->name('locations.update');
    ...
    Route::prefix('timesheet')->group(function () {
        ...
        Route::get('update_attendances/{id}/delete', [AttendanceController::class, 'updateAttendanceDelete'])->name('update_attendances.delete');
         Route::post('update_attendances/download_pdf', [AttendanceController::class, 'downloadPDF'])->name('update_attendances.download_pdf');
        Route::post('update_attendances/download_csv', [AttendanceController::class, 'downloadCSV'])->name('update_attendances.download_csv');
        ...
        Route::get('off-desk-hours', [OffDeskHoursController::class, 'index'])->name('off_desk_hours.index');
        Route::post('off-desk-hours/store', [App\Http\Controllers\OffDeskHoursController::class, 'store'])->name('off_desk_hours.store');
        Route::get('get-employee-shift', [OffDeskHoursController::class, 'getEmployeeShift'])->name('get_employee_shift');
        
        Route::delete('/off-desk-hours/{id}', [OffDeskHoursController::class, 'destroy'])->name('off_desk_hours.destroy');
    ...
    Route::post('tickets/{ticket}/assigned', [EmployeeAssignedController::class, 'employeeTicketAssigned'])->name('tickets.assigned');
        Route::post('ticket-assignments', [EmployeeAssignedController::class, 'assignEmployee'])->name('ticket.assign');
    ...
    Route::prefix('dynamic_variable')->group(function () {
        ...
        Route::post('update-leave', [EmployeeaddLeaveController::class, 'updateLeave'])->name('addLeave_employee.update_leave');
        Route::post('addLeave_employee/update', [EmployeeaddLeaveController::class, 'update'])->name('addLeave_employee.update');
        Route::get('addLeave_employee/{id}/delete', [EmployeeaddLeaveController::class, 'destroy'])->name('addLeave_employee.destroy');
        Route::resource('addLeave_employee', EmployeeaddLeaveController::class)->except(['create', 'update', 'destroy', 'show']);
    ...
    Route::prefix('file_manager')->group(function () {
        Route::get('files/view-file/{id}', [FileManagerController::class, 'view_file'])->name('files.view_file');
    ...
    Route::group(['prefix' => 'performance', 'namespace' => 'Performance'], function () {
        ...
        Route::group(['prefix' => 'appraisal'], function () {
            Route::get('appraisals/notify-evaluators', [AppraisalController::class, 'sendNotificationToEvaluators'])->name('appraisals.notify-evaluators');

            Route::get('/index', [AppraisalController::class, 'index'])->name('performance.appraisal.index');
            Route::post('/update-increment', [AppraisalController::class, 'AppraisalIncrement'])->name('performance.appraisal.updateIncrement');
...
Route::get('rem_leave_update', [LeaveTypeController::class, 'rem_leave_update'])->name('rem_leave_update');

Route::post('/update-edit-appraisal-section',[NewAppraisalController::class, 'updateEditAppraisalSection'])->name('update.edit.appraisal.section');
Route::post('/update-whole-appraisal', [NewAppraisalController::class, 'updateWholeAppraisal'])->name('updateWholeAppraisal');
Route::post('/edit-appraisal-section', [NewAppraisalController::class, 'editAppraisalSection'])->name('edit.appraisal.section');
Route::post('/delete-appraisal', [NewAppraisalController::class, 'deleteAppraisal'])->name('deleteAppraisal');


Route::post('employees/{id}/changeTeamLead', [EmployeeController::class, 'changeTeamLead'])->name('changeTeamLead');
Route::get('employee-feedback', [EmployeeController::class, 'employee_feedback'])->name('employee_feedback');
Route::get('check-employee-feedback', [EmployeeController::class, 'check_employee_feedback'])->name('check_employee_feedback');

Route::post('/store_employee_feedback', [EmployeeController::class, 'store_employee_feedback'])
    ->name('employee.feedback.store');
Route::get('get_employee_designation', [PromotionController::class, 'getEmployeeDesignation'])->name('get_employee_designation');    
Route::get('/notifications/json', [RouteClosureHandlerController::class, 'notificationsJson'])->name('notifications.json');
...
// AHHAM AI Assistant Routes
Route::prefix('ahham')->group(function () {
    Route::get('/', [AhhamController::class, 'index'])->name('ahham.index');
    Route::post('/chat', [AhhamController::class, 'chat'])->name('ahham.chat');
    Route::get('/chat-history', [AhhamController::class, 'getChatHistory'])->name('ahham.chat-history');
    Route::post('/clear-chat', [AhhamController::class, 'clearChat'])->name('ahham.clear-chat');
    Route::get('/test-api', [AhhamController::class, 'testApi'])->name('ahham.test-api');
    Route::get('/database-context', [AhhamController::class, 'getDatabaseContext'])->name('ahham.database-context');
    Route::get('/debug', function() {
        return response()->json([
            ...
            'config' => [
                'api_key_set' => !empty(config('openrouter.api_key')),
