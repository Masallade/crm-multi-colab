                    ...
                    <ul class="nav nav-tabs d-flex justify-content-between" id="myTab" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" id="employee_core_hr-tab" data-toggle="tab" href="#Employee_Core_hr"
                               role="tab" aria-controls="Employee_Core_hr" aria-selected="false">HR Operations</a>
                        </li>
                        <li class="nav-item">
                                            ...
                                            <form method="post" id="basic_sample_form" class="form-horizontal"
                                                ...
                                                <label class="text-bold">Team Lead</label>
                                                    <select name="teamlead" id="teamlead" class="selectpicker form-control"
                                                            ...
                                                            @foreach($team_lead as $team)
                                                                <option value="{{ $team->id }}" 
                                                                    {{ $employee->team_lead == $team->id ? 'selected' : '' }}>
                                                ...
                                                <div class="col-md-6 form-group">
                                                    <label>Last Appraisal Date <span class="text-danger"></span></label>
                                                    <input type="date" name="last_appraisal_date" id="last_appraisal_date"
                                                            placeholder="Last Appraisal Date"
                                                             class="form-control"
                                                            value="{{ $employee->last_appraisal_date }}">
                                                </div>
                                                    ...
                                                    <div class="col-md-4 form-group">
                                                        <label>Office Shift <span class="text-danger">*</span></label>
                                                        ...
                                                        <select name="office_shift_id" id="office_shift_id"
                                                                ...
                                                                data-live-search="true"
                                                                data-live-search-style="contains"
                                                                title="{{__('Selecting',['key'=>'Office Shift'])}}...">
...
<div class="col-md-4 form-group">
    ...
    <input 
        ...
        value="{{ $employee->joining_date ? \Carbon\Carbon::parse($employee->joining_date)->format('d-m-Y') : null }}">
</div>
...
<div class="col-md-4 form-group">
    ...
    <input 
        ...
        autocomplete="off" 
        class="form-control date"
        value="{{ $employee->probation_period ? \Carbon\Carbon::parse($employee->probation_period)->format('d-m-Y') : null }}">
</div>
                                                    ...
                                                    <div class="col-md-4 form-group">
                                                        ...
                                                        <input type="text" name="exit_date" id="exit_date"
                                                            class="form-control date"
                                                               value="{{ $employee->exit_date ? \Carbon\Carbon::parse($employee->exit_date)->format('d-m-Y') : null }}">
                                                    </div>
                        ...
                        <div class="tab-pane fade" id="Employee_Core_hr" role="tabpanel"
                            ...
                            <div class="row mb-4">
                                <div class="col-md-12">
                                    <h5>{{__('My Resignation Status')}}</h5>
                                    <div class="card">
                                        <div class="card-body">
                                            @php
                                                $user = auth()->user();
                                            ...
                                            @if($resignation)
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <h6><strong>{{__('Resignation Details')}}</strong></h6>
                                                        <p><strong>{{__('Notice Date')}}:</strong> {{ $resignation->notice_date }}</p>
                                                        <p><strong>{{__('Resignation Date')}}:</strong> {{ $resignation->resignation_date }}</p>
                                                        <p><strong>{{__('Description')}}:</strong> {{ $resignation->description }}</p>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <h6><strong>{{__('Approval Status')}}</strong></h6>
                                                        
                                                        <!-- Overall Status -->
                                                        <div class="mb-3">
                                                            <span class="badge badge-{{ $resignation->status == 'approved' ? 'success' : ($resignation->status == 'rejected' ? 'danger' : 'warning') }} badge-lg">
                                                        ...
                                                        <!-- HR Approval Status -->
                                                        <div class="mb-2">
                                                            <strong>{{__('HR Approval')}}:</strong>
                                                            @if($resignation->hr_approved)
                                                                <span class="badge badge-success">{{__('Approved')}}</span>
                                                                @if($resignation->hr_approved_at)
                                                                    <small class="text-muted">({{ \Carbon\Carbon::parse($resignation->hr_approved_at)->format('d-m-Y H:i') }})</small>
                                                                @endif
                                                            @else
                                                        ...
                                                        <div class="mb-2">
                                                            ...
                                                            @if($resignation->admin_approved)
                                                                <span class="badge badge-success">{{__('Approved')}}</span>
                                                                @if($resignation->admin_approved_at)
                                                                    <small class="text-muted">({{ \Carbon\Carbon::parse($resignation->admin_approved_at)->format('d-m-Y H:i') }})</small>
                                                                @endif
                                                            @else
                                                                <span class="badge badge-warning">{{__('Pending')}}</span>
                                                            @endif
                                                        ...
                                                        <!-- Approval Notes -->
                                                        @if($resignation->hr_approval_notes)
                                                            <div class="mt-3">
                                                                <strong>{{__('HR Notes')}}:</strong>
                                                                <p class="text-muted">{{ $resignation->hr_approval_notes }}</p>
                                                            </div>
                                                        @endif
                                                        
                                                        @if($resignation->admin_approval_notes)
                                                            <div class="mt-3">
                                                                <strong>{{__('Admin Notes')}}:</strong>
                                                                <p class="text-muted">{{ $resignation->admin_approval_notes }}</p>
                                                            </div>
                                                        @endif
                                                    </div>
                                                </div>
                                                ...
                                                <!-- Progress Bar -->
                                                <div class="mt-4">
                                                    <h6><strong>{{__('Approval Progress')}}</strong></h6>
                                                    <div class="progress" style="height: 25px;">
                                                        ...
                                                        @endphp
                                                        <div class="progress-bar bg-{{ $progress == 100 ? 'success' : ($progress > 0 ? 'info' : 'warning') }}" 
                                                    ...
                                                    <div class="mt-2">
                                                        <small class="text-muted">
                                                            @if($progress == 100)
                                                                {{__('Your resignation has been fully approved!')}}
                                                            @elseif($progress == 50)
                                                                {{__('HR has approved. Waiting for Admin approval.')}}
                                                            @else
                                                                {{__('Waiting for HR and Admin approval.')}}
                                                            @endif
                                                        </small>
                                                    </div>
                                                </div>
                                            @else
                                                <div class="text-center">
                                                    <p class="text-muted">{{__('No resignation submitted yet.')}}</p>
                                                    <a href="{{ route('resignations.index') }}" class="btn btn-primary">
    ...
    $('#basic_sample_form').on('submit', function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#form_result').html(html).slideDown(300).delay(2000).slideUp(300, function () {
                    // Redirect after showing the success message
                    window.location.href = '<?=url('staff/employees');?>';
                });
            }
        });
    });
