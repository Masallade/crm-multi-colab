        ...
        <div id="designationGroupsContainer">
            ...
            @foreach($groupedSections as $designation => $designationSections)
                <div class="designation-group mb-4">
                    <div class="designation-header" data-bs-toggle="collapse" data-bs-target="#designation-{{ str_replace(' ', '-', $designation) }}" aria-expanded="true">
                        <div class="d-flex justify-content-between w-100">
                            <div>
                                <div class="d-flex align-items-center">
                                    <i class="bi bi-people-fill me-2"></i>
                                    <h2 class="designation-title mb-0">Designation: {{ $designation }}</h2>
                                    <span class="section-count ms-2">({{ count($designationSections) }} sections)</span>
                                </div>
                                <div class="department-container mt-2">
                                    @foreach($designationSections[0]['department_names'] as $departmentName)
                                        <span class="department-badge me-2">{{ $departmentName }}</span>
                                    @endforeach
                                </div>
                            </div>
                            <div class="d-flex align-items-center">
                                <form action="{{ route('deleteAppraisal') }}" method="post" class="me-2">
                                    @csrf
                                ...
                                <form method="post" action="{{ route('updateWholeAppraisal') }}">
                                    @csrf
                                    <input type="hidden" name="sections" value='@json($designationSections, JSON_HEX_APOS)'>
                        ...
                        <div class="designation-sections">
                            @foreach($designationSections as $section)
                                <div class="section-container mb-3">
                                    <div class="section-header d-flex justify-content-between align-items-center" data-bs-toggle="collapse" data-bs-target="#section-{{ $section['id'] }}">
                                        <div class="d-flex align-items-center">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <form method="post" action="{{ route('edit.appraisal.section') }}" class="m-0 me-2">
                                                @csrf
