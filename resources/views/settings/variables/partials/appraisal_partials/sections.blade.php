...
<div class="section-container" data-section-id="{{ $section['id'] }}">
    ...
    <input type="hidden" name="sections[{{ $section['id'] }}][company_id]" value="{{ $section['company_id'] }}" class="section-title-input" required>
    ...
    <div class="section-header">
        <div class="section-title-wrapper">
            ...
            <div>
                ...
                <input type="text" name="sections[{{ $section['id'] }}][name]" value="{{ $section['name'] }}" class="section-title-input" required>
            </div>

            <div>
                <label for="">Section Weightage (%)</label>
                <input type="number" name="sections[{{ $section['id'] }}][weightage]" value="{{ $section['weightage'] }}" min="0" max="100" class="section-title-input section-weightage" onchange="validateWeightage()" required>
            </div>
        </div>
    </div>
                ...
                <div class="department-evaluator-container">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody id="evaluationMatrixBody_{{ $section['id'] }}">
                                @php
                                    // Get department and evaluator arrays in original order
                                ...
                                @if(!empty($departmentIds))
                                    @foreach($departmentIds as $index => $deptId)
                                        <tr class="department-evaluator-row">
                                            <td>
                                                <select name="sections[{{ $section['id'] }}][departments][]" class="form-select department-dropdown">
                                                    <option value="">Select Department</option>
                                                    @foreach($departments as $department)
                                                        <option value="{{ $department->id }}" {{ $deptId == $department->id ? 'selected' : '' }}>
                                            ...
                                            <td>
                                                <select name="sections[{{ $section['id'] }}][evaluators][]" class="form-select evaluator-dropdown">
                                                    <option value="">Select Evaluator</option>
                                                    @foreach($employees as $employee)
                                                        <option value="{{ $employee->id }}" {{ (isset($evaluatorIds[$index]) && $evaluatorIds[$index] == $employee->id) ? 'selected' : '' }}>
                    ...
                    <div class="mt-2 d-flex">
                        <button type="button" class="btn btn-sm btn-primary me-2" onclick="addDepartmentEvaluatorRow('{{ $section['id'] }}')">
                            <i class="bi bi-plus-circle"></i> Add Department-Evaluator Pair
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-danger" onclick="removeLastDepartmentEvaluatorPair('{{ $section['id'] }}')">
                            <i class="bi bi-dash-circle"></i> Remove Last Pair
                        </button>
                    </div>
                    
                    <!-- Hidden field to store the final department-evaluator mapping -->
                    <input type="hidden" id="departmentEvaluatorMapping_{{ $section['id'] }}" 
                           name="sections[{{ $section['id'] }}][department_evaluator_mapping]" value="">
                </div>
            </div>

            <!-- This div will contain all the indicators -->
            <div class="indicator-list">
                @foreach($section["performance_indicator"] as $indicator)
                    <div class="indicator-item" data-indicator-id="{{ $indicator['id'] }}">
                        ...
                        <input type="text" name="sections[{{ $section['id'] }}][performance_indicator][{{ $loop->index }}][name]" value="{{ $indicator['name'] }}" class="indicator-input" required>

                        <!-- Hidden Indicator ID -->
                        <input type="hidden" name="sections[{{ $section['id'] }}][performance_indicator][{{ $loop->index }}][id]" value="{{ $indicator['id'] }}">
...
function addNewSection() {
    ...
    const newSectionHTML = `
        <div class="section-container" data-section-id="${tempId}">
            ...
            <input type="hidden" name="sections[${tempId}][company_id]" value="{{ isset($section) ? $section['company_id'] : '' }}" class="section-title-input" required>
...
function addDepartmentEvaluatorRow(sectionId) {
    ...
    @foreach($departments as $department)
        const deptOption{{ $department->id }} = document.createElement('option');
        deptOption{{ $department->id }}.value = '{{ $department->id }}';
        deptOption{{ $department->id }}.textContent = '{{ $department->department_name }}';
        departmentSelect.appendChild(deptOption{{ $department->id }});
    @endforeach
    ...
    @foreach($employees as $employee)
        const empOption{{ $employee->id }} = document.createElement('option');
        empOption{{ $employee->id }}.value = '{{ $employee->id }}';
        empOption{{ $employee->id }}.textContent = '{{ $employee->first_name }} {{ $employee->last_name }}';
        evaluatorSelect.appendChild(empOption{{ $employee->id }});
    @endforeach
