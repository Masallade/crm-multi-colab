        ...
        <div class="col-md-4 mb-4">
            <div class="card h-100 border shadow-sm d-flex flex-column justify-content-between">
                <div class="card-body d-flex flex-column justify-content-center align-items-center">
                    ...
                    <p class="card-text text-center">Staff ID: {{ $employee['staff_id'] }}</p>
                    <p class="card-text text-center mb-1">Last Clock Up: {{ $employee['clock_up'] }}</p>
                    @if($employee['status'] === 'Active' && empty($hide_stream_button))
                        <button class="btn btn-primary mt-2" onclick="openImageStreamModal('{{ $employee['staff_id'] }}', {{ $employee['employee_id'] }})">View Stream</button>
                    @endif
                </div>
                <div class="card-footer bg-white border-0 text-center">
                    <span class="badge {{ $employee['status'] === 'Active' ? 'badge-success' : 'badge-secondary' }}" style="font-size: 1.1em;">
                        {{ $employee['status'] }}
                    </span>
                </div>
            </div>
